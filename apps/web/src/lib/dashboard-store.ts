...
export const dialog = {
  ...
  async prompt(title: string, description: string, defaultValue = ""): Promise<string | null> {
    return new Promise((resolve) => {
      const overlay = document.getElementById("custom-dialog-overlay")!;
      const content = document.getElementById("custom-dialog-content")!;
      const titleEl = document.getElementById("custom-dialog-title")!;
...
export const blogStore = {
  state: {
    ...
    searchQuery: "",
    statusFilter: "all",
    currentEditingPostId: null as string | null,
  ...
  setFilters(searchQuery: string, statusFilter: string) {
  ...
  createPost() {
    ...
    (document.getElementById("post-slug-input") as HTMLInputElement).value = "";
    (document.getElementById("post-published-checkbox") as HTMLInputElement).checked = false;
    ...
    this.renderAuthorSelect((window as any).currentUser ? (window as any).currentUser.id : "");
    ...
    (window as any).initTiptap?.();
    if ((window as any).editor) {
      (window as any).editor.commands.setContent("");
    }

    (window as any).showView("view-blog-editor");
  },
  ...
  async editPost(id: string) {
    try {
      ...
      (document.getElementById("post-title-input") as HTMLInputElement).value = post.title;
      (document.getElementById("post-slug-input") as HTMLInputElement).value = post.slug;
      (document.getElementById("post-published-checkbox") as HTMLInputElement).checked = post.published;

      this.state.selectedCategoryIds = post.categories ? post.categories.map((c: any) => c.id) : [];
      this.state.selectedTagIds = post.tags ? post.tags.map((t: any) => t.id) : [];
      ...
      (window as any).initTiptap?.();
      if ((window as any).editor) {
        (window as any).editor.commands.setContent(post.content || "");
      }
  ...
  async savePost(forcePublish?: boolean) {
    const title = (document.getElementById("post-title-input") as HTMLInputElement).value.trim();
    let slug = (document.getElementById("post-slug-input") as HTMLInputElement).value.trim();
    
    let published = (document.getElementById("post-published-checkbox") as HTMLInputElement).checked;
    if (forcePublish) {
      published = true;
      (document.getElementById("post-published-checkbox") as HTMLInputElement).checked = true;
    }
    ...
    const content = (window as any).editor ? (window as any).editor.getHTML() : "";
    ...
    const payload = {
      ...
      authorId: (document.getElementById("post-author-select") as HTMLSelectElement | null)?.value || undefined,
    ...
    try {
      ...
      (window as any).showView("view-blog-posts");
      await this.loadPosts();
  ...
  renderSeoPreviewImage(containerId: string, url: string) {
  ...
  updateSeoPreviews() {
    ...
    const postTitle = (document.getElementById("post-title-input") as HTMLInputElement | null)?.value.trim() || "";
    const slug = (document.getElementById("post-slug-input") as HTMLInputElement | null)?.value.trim() || "";
    ...
    try {
      if (canonical) domain = new URL(canonical).hostname.replace(/^www\./, "");
    } catch { /* keep default */ }

    const mtCount = document.getElementById("seo-meta-title-count");
    const mdCount = document.getElementById("seo-meta-desc-count");
    if (mtCount) mtCount.textContent = String(val("seo-meta-title").length);
    if (mdCount) mdCount.textContent = String(val("seo-meta-description").length);
  ...
  renderAuthorSelect(selectedId?: string) {
    const select = document.getElementById("post-author-select") as HTMLSelectElement | null;
    if (!select) return;

    const currentVal = selectedId || select.value || ((window as any).currentUser ? (window as any).currentUser.id : "");
    ...
    select.innerHTML = this.state.users.map((u: any) => {
      const label = u.name || u.email;
      return `<option value="${u.id}">${label}</option>`;
    }).join("");

    if (currentVal && this.state.users.some((u: any) => u.id === currentVal)) {
  ...
  renderSelectedChips(containerId: string, selected: { id: string; name: string }[], prefix: string, onRemove: (id: string) => void) {
  ...
  updateCategoryDropdownUI() {
    ...
    items.forEach(item => {
      ...
      if (isSel) selected.push({ id, name });
    });
  ...
  renderPosts() {
    ...
    if (isTrashView) {
      filteredPosts = filteredPosts.filter((p: any) => p.trashed === true);
    } else {
      filteredPosts = filteredPosts.filter((p: any) => p.trashed === false);
      if (this.state.statusFilter === "published") {
        filteredPosts = filteredPosts.filter((p: any) => p.published === true);
      } else if (this.state.statusFilter === "draft") {
        filteredPosts = filteredPosts.filter((p: any) => p.published === false);
      }
    }
    ...
    const prevBtn = document.getElementById("btn-posts-prev") as HTMLButtonElement | null;
    const nextBtn = document.getElementById("btn-posts-next") as HTMLButtonElement | null;
    ...
    if (totalPosts === 0) {
      ...
      if (totalEl) totalEl.textContent = totalPosts.toString();
    ...
    tableBody.innerHTML = pagePosts.map((post: any) => {
      ...
      const categoriesList = post.categories && post.categories.length > 0 
        ? `<div class="flex flex-wrap gap-1.5">${post.categories.map((c: any) => `<span class="px-2 py-0.5 rounded-sm border border-border bg-muted/40 text-foreground/90 font-medium text-xs whitespace-nowrap">${c.name}</span>`).join("")}</div>`
        : `<span class="text-muted-foreground/50 text-sm">—</span>`;
        
      const tagsStr = post.tags && post.tags.length > 0
        ? post.tags.map((t: any) => `<span class="px-1.5 py-0.5 rounded-sm bg-muted/65 text-muted-foreground text-xs font-medium border border-transparent whitespace-nowrap">#${t.name}</span>`).join(" ")
        : `<span class="text-muted-foreground/50 text-sm">—</span>`;
  ...
  renderCategories() {
    ...
    tableBody.innerHTML = this.state.categories.map((c: any) => `
    ...
    tableBody.querySelectorAll(".btn-delete-category").forEach(btn => {
      btn.addEventListener("click", async () => {
        ...
        const confirmed = await dialog.confirm(
          "Delete Category",
          "Are you sure you want to permanently delete this category? Associated posts will remain uncategorized."
        );
        if (confirmed) {
  ...
  renderCategorySelect() {
    ...
    menu.innerHTML = this.state.categories.map((c: any) => `
  ...
  renderTags() {
    ...
    tableBody.innerHTML = this.state.tags.map((t: any) => `
      <tr class="hover:bg-muted/30 transition-colors">
  ...
  renderTagCheckboxes() {
    ...
    menu.innerHTML = this.state.tags.map((t: any) => `
